...
template <class It, class UnarFunc, class BinarFun, typename Res>
void map(It p, It q, UnarFunc f1, BinarFun f2, Res* res)
{
    *res = f1(*p);
    while(++p != q)
        *res = f2(*res, f1(*p));
...
auto  map_reduce(It p, It q, UnarFunc f1, BinarFun f2,  size_t threads) -> decltype(f1(*p))
{
    ...
    for(size_t i=0; i <threads; ++i ) {
        ...
        thr[i] = std::thread(map<It, UnarFunc, BinarFun, Res>, p, end, f1, f2, &results[i] );
